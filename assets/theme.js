...
var PageDots = class extends HTMLElement {
  ...
  select(selectedIndex, dispatchEvent = true) {
    // 🚨 HACK APPLIED: The auto-scrolling math has been completely removed.
    // Thumbnails will update their active state but will NOT jump/scroll automatically.
...
var ScrollProgress = class extends HTMLElement {
  connectedCallback() {
    this._onScroll = throttle(this._updateScrollProgress.bind(this));
    this._abortController = new AbortController();
    this._resizeObserver = null;
    this._bindTargetAttempts = 0;
    ...
    // Draggable scrollbar behavior (Swiper-like)
    this.addEventListener("pointerdown", this._onPointerDown, { signal: this._abortController.signal });
  }
  disconnectedCallback() {
    this._abortController.abort();
  ...
  get scrolledElement() {
    const id = this.getAttribute("observes");
    if (!id) {
      return null;
    }
    return this._scrolledElement = this._scrolledElement || document.getElementById(id);
  }
  _bindTarget() {
    const el = this.scrolledElement;
    if (!el) {
      this._bindTargetAttempts += 1;
      if (this._bindTargetAttempts <= 100) {
        requestAnimationFrame(() => this._bindTarget());
      }
      return;
    }
    el.addEventListener("scroll", this._onScroll, { passive: true, signal: this._abortController.signal });
    el.addEventListener("scrollend", this._onScroll, { passive: true, signal: this._abortController.signal });
    if (window.ResizeObserver) {
  ...
  _updateScrollProgress() {
    ...
    const scrollLeft = document.dir === "ltr" ? el.scrollLeft : Math.abs(el.scrollLeft);
    const { clientWidth, scrollWidth } = el;
    if (!scrollWidth || !clientWidth) {
      return;
    }
    if (scrollWidth <= clientWidth) {
      this.style.setProperty("--scroll-progress", "1");
      return;
    }
    // Represent the current scroll position (0..1) so the thumb can be dragged.
    const maxScroll = scrollWidth - clientWidth;
    const ratio = maxScroll > 0 ? scrollLeft / maxScroll : 1;
    const clamped = Math.max(0, Math.min(ratio, 1));
    if (Number.isFinite(clamped)) {
      this.style.setProperty("--scroll-progress", clamped.toFixed(6));
    }
  }
  ...
  _scrollToRatio(ratio) {
    ...
    const maxScroll = scrollWidth - clientWidth;
    const clamped = Math.max(0, Math.min(ratio, 1));
    const targetLeft = clamped * maxScroll;

    // RTL in many browsers uses negative scrollLeft.
    const left = document.dir === "ltr" ? targetLeft : -targetLeft;
  ...
  _onPointerDown(event) {
    ...
    event.preventDefault();
    this._scrollToRatio(this._ratioFromEvent(event));

    this.addEventListener("pointermove", this._onPointerMove, { signal: this._abortController.signal });
    this.addEventListener("pointerup", this._onPointerUp, { signal: this._abortController.signal });
    this.addEventListener("pointercancel", this._onPointerUp, { signal: this._abortController.signal });
    this.addEventListener("lostpointercapture", this._onPointerUp, { signal: this._abortController.signal });
  }

  _onPointerMove(event) {
    if (!this._isPointerDown || event.pointerId !== this._activePointerId) {
      return;
    }
    event.preventDefault();
    this._scrollToRatio(this._ratioFromEvent(event));
...
var ScrollCarousel = class extends BaseCarousel {
  constructor() {
    ...
    this._onDragPointerEnd = this._onDragPointerEnd.bind(this);
    this._flushDragScrollFrame = this._flushDragScrollFrame.bind(this);
  }
  connectedCallback() {
    ...
    this.addEventListener("pointerdown", this._onDragPointerDown, { signal: this._abortController.signal });
    this.addEventListener("pointermove", this._onDragPointerMove, { signal: this._abortController.signal });
    this.addEventListener("pointerup", this._onDragPointerEnd, { signal: this._abortController.signal });
    this.addEventListener("pointercancel", this._onDragPointerEnd, { signal: this._abortController.signal });
    this.addEventListener("lostpointercapture", this._onDragPointerEnd, { signal: this._abortController.signal });
  }
  disconnectedCallback() {
    this._cancelDragMomentum();
    if (this._dragRafId) {
  ...
  _cancelDragMomentum() {
    if (this._dragMomentumRaf) {
      cancelAnimationFrame(this._dragMomentumRaf);
      this._dragMomentumRaf = null;
    }
    this.classList.remove("is-momentum-scroll");
    if (this.style.scrollSnapType === "none") {
      this.style.scrollSnapType = "";
    }
  }
  _flushDragScrollFrame() {
    this._dragRafId = null;
    if (this._dragPointerId === null || !this._dragMoved) {
      return;
    }
    const dx = this._dragPendingClientX - this._dragStartClientX;
    this.scrollLeft = this._dragStartScrollLeft - dx;
  }
  _startDragMomentum(initialVelocityPxPerMs) {
    this._cancelDragMomentum();
    let v = initialVelocityPxPerMs;
    let last = performance.now();
    this.classList.add("is-momentum-scroll");
    this.style.scrollSnapType = "none";
    const tick = (now) => {
      const dt = now - last;
      last = now;
      const max = this.scrollWidth - this.clientWidth;
      let next = this.scrollLeft + v * dt;
      if (next <= 0) {
        this.scrollLeft = 0;
        this._endDragMomentum();
        return;
      }
      if (next >= max) {
        this.scrollLeft = max;
      ...
      this._dragMomentumRaf = requestAnimationFrame(tick);
    };
    this._dragMomentumRaf = requestAnimationFrame(tick);
  }
  _endDragMomentum() {
    this._dragMomentumRaf = null;
    this.classList.remove("is-momentum-scroll");
    this._snapToNearestAfterDrag();
  }
  /**
   * Same scroll target as _transitionTo, used to pick the nearest snap position after a free drag.
   */
  _snapScrollLeftForItem(slide) {
    const align = this._extractSlideAlign(slide);
    if (align === "center") {
      return slide.offsetLeft - (this.clientWidth / 2 - (parseInt(getComputedStyle(this).scrollPaddingInline) || 0)) + slide.clientWidth / 2;
    }
    // FIX APPLIED HERE
    return slide.offsetLeft - (parseInt(getComputedStyle(this).scrollPaddingInline) || 0);
  }
  /**
   * Jump to the nearest slide in one instant scroll. Re-applying scroll-snap alone makes the browser
   * smooth-scroll with `scroll-behavior: smooth` and honor `scroll-snap-stop: always`, which steps one slide at a time.
   */
  _snapToNearestAfterDrag() {
    ...
    let bestItem = this.visibleItems[0];
    for (const item of this.visibleItems) {
      let left = this._snapScrollLeftForItem(item);
      left = Math.max(0, Math.min(maxScroll, left));
      const dist = Math.abs(this.scrollLeft - left);
      if (dist < bestDist) {
        bestDist = dist;
        bestLeft = left;
        bestItem = item;
    ...
    this.style.scrollSnapType = "none";
    this.scrollTo({ left: bestLeft, behavior: "auto" });
    this.style.scrollSnapType = "";
    const idx = this.items.indexOf(bestItem);
    if (idx >= 0) {
  ...
  _onDragPointerDown(event) {
    if (this._hasPendingProgrammaticScroll) {
      return;
    }
    if (this._scrollArea.scrollDirection !== "inline") {
      return;
    }
    ...
    if (event.target.closest("button, a[href], input, textarea, select, label, summary, [role='button']")) {
      return;
    }
    this._cancelDragMomentum();
    ...
    this._dragStartScrollLeft = this.scrollLeft;
  }
  _onDragPointerMove(event) {
    if (event.pointerId !== this._dragPointerId || this._dragPointerId === null) {
      return;
    }
    const dx = event.clientX - this._dragStartClientX;
    const dy = event.clientY - this._dragStartClientY;
    if (!this._dragMoved) {
      ...
      this.style.scrollSnapType = "none";
      this.classList.add("is-pointer-dragging");
    }
    event.preventDefault();
    const now = performance.now();
    this._dragVelocitySamples.push({ t: now, x: event.clientX });
    while (this._dragVelocitySamples.length > 0 && now - this._dragVelocitySamples[0].t > 90) {
      this._dragVelocitySamples.shift();
    }
    this._dragPendingClientX = event.clientX;
    if (!this._dragRafId) {
      this._dragRafId = requestAnimationFrame(this._flushDragScrollFrame);
    }
  }
  _onDragPointerEnd(event) {
    ...
    if (dragMoved) {
      ...
      this.classList.remove("is-pointer-dragging");
      const stopAccidentalClick = (clickEvent) => {
        clickEvent.preventDefault();
        clickEvent.stopPropagation();
      };
      this.addEventListener("click", stopAccidentalClick, { capture: true, once: true });
      let scrollVelocity = 0;
      const samples = this._dragVelocitySamples;
      if (samples.length >= 2) {
        const first = samples[0], last = samples[samples.length - 1];
        const dt = last.t - first.t;
        if (dt > 0) {
          scrollVelocity = -(last.x - first.x) / dt;
        }
      }
      this._dragVelocitySamples = [];
      if (Math.abs(scrollVelocity) >= 0.12) {
        this._startDragMomentum(scrollVelocity);
      } else {
        this._snapToNearestAfterDrag();
      }
    }
  ...
  select(index, { animate: animate19 = true, force = false } = {}) {
    ...
    // FIX APPLIED HERE
    index = this.items.indexOf(this.visibleItems[Math.max(0, indexAmongVisible)]);
  ...
  _transitionTo(fromSlide, toSlide, { animate: animate19 = true } = {}) {
    ...
    switch (slideAlign) {
      case "start":
        // FIX APPLIED HERE
        scrollAmount = toSlide.offsetLeft - (parseInt(getComputedStyle(this).scrollPaddingInline) || 0);
        break;
      case "center":
    ...
    if (this._rafScrollFrame) {
      cancelAnimationFrame(this._rafScrollFrame);
      this._rafScrollFrame = null;
    }
    const previousSnapType = this.style.scrollSnapType;
    if (!animate19) {
      this.style.scrollSnapType = "none";
      this._hasPendingProgrammaticScroll = false;
      this.scrollTo({ left: scrollAmount, behavior: "auto" });
      this.style.scrollSnapType = previousSnapType;
      this._onScrollSettled();
      return;
    }
    this._hasPendingProgrammaticScroll = true;
    this.style.scrollSnapType = "none";
    ...
    const step = (now) => {
      ...
      this.scrollLeft = startScroll + distance * easeInOutQuad(progress);
      if (progress < 1) {
        this._rafScrollFrame = requestAnimationFrame(step);
      } else {
        this._rafScrollFrame = null;
  ...
  _onCarouselScroll() {
    ...
    // FIX APPLIED HERE
    const newIndex = this.items.indexOf(this.visibleItems[Math.min(this.visibleItems.length - 1, Math.round(this.scrollLeft / this.itemOffset))]);
  ...
  _adjustHeight() {
    ...
    if (this.hasAttribute("adaptive-height") && this.selectedSlide) {
      const newHeight = this.selectedSlide.clientHeight;
      if (newHeight > 0) {
...
var FreeShippingBar = class extends HTMLElement {
  ...
  _updateMessage() {
    ...
    const helperTextElement = this.querySelector(".free-shipping-bar__helper-text");
    const helperAmountElement = this.querySelector(".free-shipping-bar__helper-amount");
    const formattedThreshold = this._currencyFormatter.format(this.threshold / 100).replace(/\$/g, "$$");
    if (this.totalPrice >= this.threshold) {
      this.classList.add("is-reached");
      messageElement.innerHTML = this.getAttribute("reached-message");
    } else {
      this.classList.remove("is-reached");
      const formattedAmount = this._currencyFormatter.format((this.threshold - this.totalPrice) / 100);
      const replacement = `<span class="bold text-accent">${formattedAmount.replace(/\$/g, "$$$$")}</span>`;
      messageElement.innerHTML = this.getAttribute("unreached-message").replace(new RegExp("({{.*}})", "g"), replacement);
    }
    if (helperTextElement) {
      const helperTemplate = this.getAttribute("helper-message-template") || "";
      helperTextElement.textContent = helperTemplate.replace("[[amount]]", "").replace(/\s+/g, " ").trim();
    }
    if (helperAmountElement) {
      helperAmountElement.textContent = formattedThreshold;
    }
  }
...
var LineItemQuantity = class extends HTMLElement {
  ...
  _onRemoveLinkClicked(event) {
    ...
    if (quantityButton) {
      event.preventDefault();
      const quantityInput = this.querySelector('.quantity-input[data-line-key]');
      if (!quantityInput) {
        return;
      }
      const step = parseInt(quantityInput.step) || 1, min = parseInt(quantityInput.min) || 0, max = quantityInput.max ? parseInt(quantityInput.max) : Infinity, currentValue = parseInt(quantityInput.value) || 0;
      let targetQuantity = currentValue;
      if (quantityButton.getAttribute("data-quantity-action") === "increase") {
        targetQuantity = currentValue + step;
      } else {
        targetQuantity = currentValue - step;
      }
      targetQuantity = Math.max(min, Math.min(max, targetQuantity));
      if (targetQuantity === currentValue) {
        return;
      }
      quantityInput.value = targetQuantity.toString();
      this._changeLineItemQuantity(quantityInput.getAttribute("data-line-key"), targetQuantity);
      return;
    }
...
// Swiper sliders (featured tabs, related products, testimonials, origin review slider)
(() => {
  const getControlScope = (swiperEl) => swiperEl.closest('[role="tabpanel"]') || swiperEl.closest(".shopify-section") || document;
  const setButtonDisabled = (button, disabled) => {
    if (!button) return;
    button.disabled = Boolean(disabled);
    button.toggleAttribute("disabled", Boolean(disabled));
  };
  const wireDisabledState = (swiper, prevEl, nextEl) => {
    const update = () => {
      setButtonDisabled(prevEl, swiper.isBeginning);
      setButtonDisabled(nextEl, swiper.isEnd);
    };
  ...
  const shouldDisableSwiper = (swiperEl) => {
    const type = swiperEl.getAttribute("data-swiper");
    return (type === "featured-collections-tabs" || type === "related-products") && window.matchMedia("(max-width: 999px)").matches;
  };
  const initOne = (swiperEl) => {
    const SwiperCtor = window.Swiper;
    if (!SwiperCtor) return;
    if (!(swiperEl instanceof Element)) return;
    if (!swiperEl.isConnected) return;
    if (shouldDisableSwiper(swiperEl)) {
      if (swiperEl.swiper) {
    ...
    const id = swiperEl.getAttribute("id");
    const sliderType = swiperEl.getAttribute("data-swiper");
    const scope = getControlScope(swiperEl);
    const prevEl = id ? scope.querySelector(`[aria-controls="${CSS.escape(id)}"][data-swiper-prev]`) : scope.querySelector("[data-swiper-prev]");
    const nextEl = id ? scope.querySelector(`[aria-controls="${CSS.escape(id)}"][data-swiper-next]`) : scope.querySelector("[data-swiper-next]");
    const scrollbarEl = scope.querySelector("[data-swiper-scrollbar]");

    const cs = getComputedStyle(swiperEl);
    const parsePx = (value) => {
      const n = parseFloat(String(value || "").trim());
      return Number.isFinite(n) ? n : 0;
    };
    const spaceBetween = (() => {
      if (sliderType === "featured-collections-tabs" || sliderType === "related-products") {
        return parsePx(cs.getPropertyValue("--product-list-column-gap")) || parsePx(cs.getPropertyValue("--grid-gutter")) || 0;
      }
      return parsePx(cs.getPropertyValue("--grid-gutter")) || 0;
    })();
    const paddingInlineEnd = parsePx(cs.getPropertyValue("padding-inline-end")) || parsePx(cs.getPropertyValue("padding-right")) || 0;

    const options = {
      slidesPerView: "auto",
      spaceBetween,
      slidesOffsetAfter: spaceBetween + paddingInlineEnd,
      freeMode: { enabled: true, sticky: false },
      watchOverflow: true,
      grabCursor: true,
      resistanceRatio: 0.65,
      scrollbar: scrollbarEl instanceof Element ? { el: scrollbarEl, draggable: true } : void 0,
      navigation: prevEl instanceof Element || nextEl instanceof Element ? { prevEl, nextEl } : void 0
    };
    if (sliderType === "origin-review") {
      options.breakpoints = {
        0: {
          slidesPerView: 1.3,
          spaceBetween: 10,
    ...
    try {
      const swiper = new SwiperCtor(swiperEl, options);
      wireDisabledState(swiper, prevEl, nextEl);
      return swiper;
    } catch (e) {
      return;
    }
  };
  const syncResponsiveSwipers = (root = document) => {
    root.querySelectorAll("[data-swiper]").forEach((el) => {
      if (shouldDisableSwiper(el)) {
  ...
  const initAll = (root = document) => {
    ...
    root.querySelectorAll("[data-swiper]").forEach((el) => initOne(el));
  };
  const updateVisibleTabpanelSwipers = (tabsRoot) => {
    const visiblePanels = Array.from(tabsRoot.querySelectorAll('[role="tabpanel"]')).filter((p) => !p.hasAttribute("hidden"));
    visiblePanels.forEach((panel) => {
      panel.querySelectorAll("[data-swiper]").forEach((el) => {
        if (el.swiper) {
          el.swiper.update();
        } else {
          initOne(el);
  ...
  document.addEventListener("shopify:section:unload", (event) => {
    event.target.querySelectorAll("[data-swiper]").forEach((el) => el.swiper?.destroy(true, true));
  });
  window.addEventListener("resize", throttle(() => syncResponsiveSwipers(document)));
  ...
  document.addEventListener("click", (event) => {
    const tabButton = event.target?.closest?.(".featured-collections-tabs .tabs-nav__button");
    if (!tabButton) return;
    const tabsRoot = tabButton.closest(".featured-collections-tabs");
    if (!tabsRoot) return;
    requestAnimationFrame(() => updateVisibleTabpanelSwipers(tabsRoot));
  });
})();
export {
  AccordionDisclosure,
